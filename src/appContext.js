...
export default function WrapContext({ children }) {
 
  //  Storing API in Const state
  const [state, setState] = useState(store);


  // data is Attribute in Which our Downloaded File gets Passed and Through setState function/Hook
  // the data or API,js file gets updated to state varible and rendered on Screen
  ...
  //  API.js or JSON data gets updated to a varible newState.
    const newState = state;


    if (dragIds.length === 1) {
    
      const getDragChapterIndex = newState.childrenAllIdsOrder.findIndex((id) => id === dragIds[0]);

      const getDropChapterIndex = newState.childrenAllIdsOrder.findIndex((id) => id === dropIds[0]);

      const swapId = newState.childrenAllIdsOrder.splice(getDragChapterIndex ,1);
  ...
  const HandleChangeStandard = (event, chapterId, headingId, subheadingId) => {
    ...
    if (chapterId && headingId && subheadingId) {
      newState.children[chapterId].children[headingId].children[subheadingId].name = value;
